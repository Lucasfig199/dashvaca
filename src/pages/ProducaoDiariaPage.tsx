...
import { registrarProducaoDiariaNoHistorico } from "@/lib/historicoUtils";
...
export default function ProducaoDiariaPage() {
  ...
  const carregarHoje = async () => {
    try {
      ...
      const url = `${SUPABASE_URL}/rest/v1/PROD-LEITE?select=${selectCols}&"DATA-MED"=eq.${todayYMD()}&CLASSE=eq.PROD-DIA`;
      ...
      const total = rows.reduce((a: number, r: any) => a + litrosToNumber(r['LITROS-MED']), 0);
  ...
  const carregarLista = async () => {
    try {
      ...
      const where = start && end ? `and=(CLASSE.eq.PROD-DIA,"DATA-MED".gte.${start},"DATA-MED".lte.${end})&` : `CLASSE=eq.PROD-DIA&`;
  ...
  const salvarRegistro = async () => {
    ...
    try {
      ...
      const codigo = await registrarProducaoDiariaNoHistorico({
        data: formData.data,
        litros: litrosNum,
    ...
    <div className="space-y-6 bg-white">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-gradient-production/30 backdrop-blur-sm border border-production-border rounded-xl p-3 sm:p-4 lg:p-6 shadow-lg gap-3 sm:gap-0">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-foreground">Produção Diária</h2>
            <p className="text-xs sm:text-sm text-muted-foreground">Gestão da produção diária</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
        ...
        {/* Filtro de período */}
        <Card className="bg-gradient-to-br from-card to-card/80 border shadow-lg sm:col-span-2 lg:col-span-2">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-start sm:items-center">
              <div className="flex-1 w-full sm:w-auto">
                <Label htmlFor="periodo-select" className="text-xs sm:text-sm mb-1 sm:mb-2 block">Período</Label>
              ...
              {periodo === 'personalizar' && (
                <>
                  <div className="flex-1 w-full sm:w-auto">
                    <Label htmlFor="data-inicio" className="text-xs sm:text-sm mb-1 sm:mb-2 block">De</Label>
                    <Input
                      type="date"
                      value={dataInicio}
                  ...
                  <div className="flex-1 w-full sm:w-auto">
                    <Label htmlFor="data-fim" className="text-xs sm:text-sm mb-1 sm:mb-2 block">Até</Label>
                    <Input
                      type="date"
                      value={dataFim}
                  ...
                  <div className="flex items-end w-full sm:w-auto">
                    <Button 
                      ...
                      className="w-full sm:w-auto bg-gradient-production hover:bg-gradient-production-hover text-foreground border border-production-border"
                    >
                      Aplicar
                    </Button>
                  </div>
      ...
      {/* Gráfico */}
      <Card className="bg-gradient-to-br from-card to-card/80 border shadow-lg">
        ...
        <CardContent className="p-2 sm:p-4 lg:p-6">
          {registros.length === 0 ? (
            <div className="text-center py-8 sm:py-12 text-muted-foreground">
              <Milk className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 opacity-50" />
              <p className="text-sm sm:text-base">Nenhum registro encontrado para o período selecionado.</p>
            </div>
          ) : (
            <div className="space-y-2 sm:space-y-3">
              <div className="hidden sm:grid sm:grid-cols-5 gap-2 sm:gap-4 p-2 sm:p-3 bg-muted/50 rounded-lg text-xs sm:text-sm font-medium text-muted-foreground">
                <div>Data</div>
                <div>Hora</div>
                <div>Litros</div>
                <div>Período</div>
              ...
              {registros.map((registro, index) => (
                <div key={index} className="grid grid-cols-1 sm:grid-cols-5 gap-1 sm:gap-4 p-3 sm:p-3 bg-card/50 rounded-lg border border-border/50 hover:bg-card/80 transition-colors">
                  <div className="flex justify-between sm:block">
                    <span className="text-xs text-muted-foreground sm:hidden">Data:</span>
                    <span className="text-xs sm:text-sm">
                      {formatDateForDisplay(registro['DATA-MED'])}
                    </span>
                  </div>
                  <div className="flex justify-between sm:block">
                    <span className="text-xs text-muted-foreground sm:hidden">Hora:</span>
                    <span className="text-xs sm:text-sm">{registro['HORA-MED']}</span>
                  </div>
                  <div className="flex justify-between sm:block">
                    <span className="text-xs text-muted-foreground sm:hidden">Litros:</span>
                    <span className="text-xs sm:text-sm font-medium">{Number(litrosToNumber(registro['LITROS-MED'])).toFixed(1)}L</span>
                  </div>
                  <div className="flex justify-between sm:block">
                    <span className="text-xs text-muted-foreground sm:hidden">Período:</span>
                    <span className="text-xs sm:text-sm">{registro['PERIODO-MED']}</span>
                  </div>
                  <div className="flex justify-between sm:block">
                    <span className="text-xs text-muted-foreground sm:hidden">Obs:</span>
                    <span className="text-xs sm:text-sm text-muted-foreground">{registro['OBS-MED'] || '-'}</span>
                  </div>
                </div>
              ))}
            </div>
  ...
  return <FarmDashboard title="Produção Diária">
    {content}
  </FarmDashboard>;
}