...
import { Milk, TrendingUp, TrendingDown, DollarSign, Users, Activity, Plus, Edit, Trash2 } from "lucide-react";
...
const Index = () => {
  ...
  // Load today's production (from Produção Diária - PROD-DIA)
  ...
  const loadMonthlyExpenses = async () => {
    try {
      ...
      const where = `and=(CLASSE.eq.GASTO,DIA.gte.${firstDay},DIA.lte.${lastDay})&`;
  ...
  const loadMonthlyProfits = async () => {
    try {
      ...
      const where = `and=(CLASSE.eq.LUCRO,DIA.gte.${firstDay},DIA.lte.${lastDay})&`;
  ...
  const calculateMonthlyBalance = async () => {
    try {
      ...
      // Get expenses
      const expenseWhere = `and=(CLASSE.eq.GASTO,DIA.gte.${firstDay},DIA.lte.${lastDay})&`;
      ...
      // Get profits
      const profitWhere = `and=(CLASSE.eq.LUCRO,DIA.gte.${firstDay},DIA.lte.${lastDay})&`;
  ...
  // Load production data for chart (last 15 days)
  const loadProductionChart = async () => {
    try {
      const start = addDays(todayYMD(), -14);
      const end = todayYMD();
  ...
  const loadRecentActivities = async () => {
    try {
      const start = addDays(todayYMD(), -7);
  ...
  const renderChart = (rows: ProductionRecord[]) => {
    ...
    if (!rows.length) {
      ...
      ctx.fillText('Sem dados de produção nos últimos 15 dias.', pad.left, pad.top + 20);
      return;
    }
      ...
      <div className="space-y-4 sm:space-y-6 lg:space-y-8">
        ...
        <Card className="bg-gradient-card border-border shadow-lg">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <h3 className="text-lg sm:text-lg font-bold text-foreground mb-3 sm:mb-4">Produção de Leite - Últimos 15 dias</h3>
